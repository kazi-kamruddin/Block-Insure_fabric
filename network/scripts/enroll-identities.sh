...
enroll_peer_org() {
  ...
  fabric-ca-client register --caname "${ca_name}" --id.name admin --id.secret adminpw --id.type admin --tls.certfiles "${ca_cert}"
  ...
  fabric-ca-client enroll -u "https://admin:adminpw@localhost:${port}" --caname "${ca_name}" -M "${base}/users/Admin@${domain}/msp" --tls.certfiles "${ca_cert}"
...
enroll_peer_org bank BankMSP 10054 bankOfficer
